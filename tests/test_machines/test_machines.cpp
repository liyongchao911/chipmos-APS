...
class test_machines_t : public testing::Test
{
private:
...
entity_t *test_machines_t::createAnEntity(map<string, string> data)
{
...
lot_t *test_machines_t::createALot(map<string, string> data)
{
    lot_t *lot = nullptr;
    lot = new lot_t(data);
    if (lot == nullptr) {
        perror("Failed to new lot_t instalce in test_machines_t::createALot");
        exit(EXIT_FAILURE);
    }
...
void test_machines_t::TearDown()
{
    delete machines;
}

TEST_F(test_machines_t, test_machines_addMachine1)
{
...
TEST_F(test_machines_t, test_machines_addMachine2)
{
    ent = createAnEntity(test_entity_case1);
