...
__qualifier__ void
job_base_reset(job_base_t *self)
{
	self->start_time = 0;
	self->end_time = 0;
}
...
__qualifier__ void set_process_time(
        job_base_t *self,
...
// getter
__qualifier__ double
get_ms_gene(job_base_t *self){ return *(self->ms_gene); }

__qualifier__ double
get_os_gene(job_base_t *self){ return *(self->os_seq_gene); }

__qualifier__ unsigned int
get_machine_no(job_base_t *self){ return self->machine_no; }

__qualifier__ double
get_arrival_time(job_base_t *self){ return self->arriv_t; }

__qualifier__ double
get_start_time(job_base_t *self){ return self->start_time; }

__qualifier__ double
get_end_time(job_base_t *self){ return self->end_time; }

// operation
__qualifier__ unsigned int
machine_selection(job_base_t *self)
{
...
{
	job_base_t *jb =
	        (job_base_t*)malloc(sizeof(job_base_t));
