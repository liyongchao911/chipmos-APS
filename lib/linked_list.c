...
__qualifier__ list_ele_t * list_merge(list_ele_t * l1, list_ele_t * l2, list_operations_t *ops){
	if(!l2) return l1;
	if(!l1) return l2;
	...
	if(l1) _list_ele_set_next(result_iter, l1);
	else if(l2) _list_ele_set_next(result_iter, l2);
	return result;

}


__qualifier__  list_ele_t * list_merge_sort(list_ele_t * head, list_operations_t *ops){
	if(!head || !head->next) {
		return head;
	}else{
