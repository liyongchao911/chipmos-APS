 ...
 *  2. The function will use get_value function pointer to evaluate each list_ele_t's value.
 *    Please make sure that get_value is not NULL and it point to the correct function before invoking this function.
 *  3. The function will use set_next function pointer to add new list node or concatenate two lists.
 *    Please make sure that set_next in @ops is point to correct function.
 *  */
__qualifier__ list_ele_t *list_merge_sort(
        list_ele_t *head,
 ...
 */
__qualifier__ list_ele_t *list_merge(
        list_ele_t *l1,
        list_ele_t *l2,
        list_operations_t *ops
);
 ...
 *
 * The list_operations_t is used to link to the functions, which we would like to perform on list element,
 * on host or on device. The library provides flexiblility on changing the function which user 
 * would like to perform on list element. The variables of list_operations_t are function pointers.
 *
 ...
 * in which, the field set_next point to _list_ele_set_next and the field set_prev point to _list_ele_set_prev
 */
#define LINKED_LIST_OPS list_operations_t{               \
	.set_next = _list_ele_set_next,                      \
	.set_prev = _list_ele_set_prev,                      \
}
#endif
